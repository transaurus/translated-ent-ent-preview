...
为确保软件质量，团队通常会实施_持续集成_工作流（即CI）。通过CI流程，团队会对代码库的每次变更自动运行一系列验证。在CI过程中，团队可能执行多种验证：

* 编译或构建最新版本以确保代码未被破坏
* 代码静态分析以强制执行公认的代码风格标准
* 单元测试验证各组件是否符合预期，且代码变更不会导致其他功能退化
* 安全扫描确保代码库未引入已知漏洞
* 以及其他更多验证！

根据与Ent社区的交流，我们了解到许多使用Ent的团队已采用CI流程，并希望在工作流中加入Ent特有的验证机制。
...
Ent高度依赖代码生成。根据我们的经验，生成的代码应始终提交至版本控制系统，主要原因有二：

* 提交生成的代码可使其与主应用程序代码一同被查阅。在代码审查或浏览仓库时，生成代码的存在对于完整理解系统工作原理至关重要
* 能及时发现并解决团队成员开发环境的差异。这进一步降低了"在我机器上能运行"类问题的发生概率，因为所有人都在运行相同的代码

若使用GitHub进行版本控制，可通过`ent/contrib/ci` GitHub Action轻松验证所有生成文件是否已提交。对于其他系统，我们提供了简单的bash脚本供集成到现有CI流程中。
...
项目Ent模式的变更几乎总会导致数据库修改。若使用[版本化迁移](/docs/versioned-migrations)管理数据库模式变更，可在持续集成流程中运行[迁移静态检查](https://atlasgo.io/versioned/lint)，原因包括：

* 静态检查会在[数据库容器](https://atlasgo.io/concepts/dev-database)上重放迁移目录，确保所有SQL语句有效且顺序正确
* 强制执行[迁移目录完整性](/docs/versioned-migrations#atlas-migration-directory-integrity-file)——确保历史记录未被意外修改，且并行规划的迁移会统一为清晰的线性历史
* 检测破坏性变更，在迁移触及生产环境前预警潜在数据丢失风险
* 识别可能依赖数据的变更，这些变更_可能_在部署时失败，需要更仔细的审查

若使用GitHub，可通过[官方Atlas Action](https://github.com/ariga/atlas-action)在CI期间执行迁移静态检查。
