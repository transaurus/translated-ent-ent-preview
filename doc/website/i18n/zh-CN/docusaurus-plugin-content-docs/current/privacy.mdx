...
隐私层的主要优势在于，您只需在schema中**一次性**编写隐私策略，该策略将**始终**生效。无论查询和变更操作在代码库的哪个位置执行，都会经过隐私层的校验。

本教程将从框架使用的基本术语开始讲解，接着介绍如何为项目配置策略功能，最后通过几个示例进行演示。
...
- `privacy.Allow` - 若从隐私规则返回，则停止评估（跳过后续规则），执行的操作（查询或变更）获得对目标节点的访问权限。
  
- `privacy.Deny` - 若从隐私规则返回，则停止评估（跳过后续规则），执行的操作被取消。效果等同于返回任意错误。
  
- `privacy.Skip` - 跳过当前规则，继续执行下一条隐私规则。效果等同于返回`nil`错误。
...
:::important
需注意：与[schema钩子](hooks.md#hooks-registration)类似，如果在schema中使用**`Policy`**选项，**必须**在主包中添加以下导入语句，因为schema包与生成的ent包之间可能存在循环引用：
...
我们从一个简单示例开始：该应用允许所有用户读取数据，但仅允许具有管理员角色的用户进行变更操作。为此我们将创建两个附加包：
...
我们定义了一个拒绝所有变更操作但接受所有查询操作的策略。但如前所述，在本例中我们仅允许具有管理员角色的查看者执行变更操作。为此需要创建两条隐私规则：
...
第一条规则`DenyIfNoViewer`确保每个操作上下文中都存在查看者，否则拒绝操作。第二条规则`AllowIfAdmin`允许具有管理员角色的查看者执行任何操作。将这些规则添加到模式并运行代码生成：
...
由于首先定义`DenyIfNoViewer`规则，该规则会优先执行，因此在`AllowIfAdmin`规则中安全访问`viewer.Viewer`对象。

添加上述规则并完成代码生成后，隐私层逻辑将作用于所有`ent.Client`操作。
...
有时需要将特定隐私决策绑定到`context.Context`。此时可使用`privacy.DecisionContext`函数创建附加隐私决策的新上下文。
...
本例将创建包含三种实体类型的模式：`Tenant`、`User`和`Group`。辅助包`viewer`和`rule`（如前所述）也用于构建应用结构。

![tenant-example](https://entgo.io/images/assets/tenant_medium.png)

让我们逐步构建这个应用。首先创建三个不同的模式（完整代码参见[此处](https://github.com/ent/ent/tree/master/examples/privacytenant/ent/schema)），由于需要在模式间共享逻辑，我们创建了[mixed-in schema](schema-mixin.md)并将其添加到所有其他模式：
...
与之前示例类似，我们需要添加约束：仅管理员用户可以创建租户（其他情况拒绝）。通过复制前述的`AllowIfAdmin`规则并添加到`Tenant`模式的`Policy`中实现：
...
继续添加数据模型中剩余的边（见上图）。由于`User`和`Group`都与`Tenant`模式存在关联边，我们创建名为`TenantMixin`的共享[mixed-in schema](schema-mixin.md)：
...
接下来可能需要添加规则：限制查看者只能查询与其所属租户关联的组和用户。针对此类场景，Ent提供称为`Filter`的特殊隐私规则类型。该规则可根据查看者身份过滤实体，不仅能返回隐私决策，还能限制查看者的查询范围。
...
创建 `FilterTenantRule` 隐私规则后，我们将其添加到 `TenantMixin` 中，以确保**所有使用该混合模式**的架构都会继承此隐私规则。
...
我们通过为 `Group` 架构添加名为 `DenyMismatchedTenants` 的隐私规则来完善示例。该规则会在关联用户与组不属于同一租户时拒绝组的创建。
...
某些情况下，我们希望在不从数据库加载实体的前提下（不同于上述 `DenyMismatchedTenants` 示例），直接拒绝用户对非所属租户实体的操作。为此，我们依赖 `FilterTenantRule` 使其在变更操作中也进行过滤，当 `tenant_id` 列与查看者上下文中存储的值不匹配时，预期操作会返回 `NotFoundError`。
