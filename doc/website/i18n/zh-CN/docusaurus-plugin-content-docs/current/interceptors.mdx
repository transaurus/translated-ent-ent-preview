...
拦截器是用于各类Ent查询的执行中间件。与钩子不同，拦截器作用于读取路径并通过接口实现，允许在不同阶段拦截并修改查询，从而提供对查询行为更细粒度的控制。例如，可参考下方的[Traverser接口](#defining-a-traverser)。
...
但通过`intercept`特性标志生成的工具，可创建适用于任意查询类型的通用拦截器。该特性标志可通过以下两种方式添加到项目：
...
:::important
需注意：与[模式钩子](hooks.md#hooks-registration)类似，若在模式中使用**`Interceptors`**选项，**必须**在主包添加以下导入语句，以避免模式包与生成的ent包之间出现循环引用：
```go
...
某些场景下需要拦截[图遍历](traversals.md)并在查询返回节点前修改构建器。例如下方查询中，我们希望确保系统中**所有**图遍历仅处理`active`用户：
...
定义并注册此类Traverser后，将对系统中所有图遍历生效。例如：
...
`Interceptors`和`Traversers`均可用于修改查询行为，但作用于不同执行阶段。拦截器作为中间件，允许在查询执行前修改查询语句，并在数据库返回记录后修改结果，因此仅作用于查询的最终阶段——实际数据库语句执行时。而遍历器在更早阶段调用，可在图遍历的每一步中修改中间查询和最终查询，然后才将它们组合起来。
...
软删除模式是拦截器与钩子的典型用例。下例展示如何通过[`ent.Mixin`](schema-mixin.md)为项目中所有模式添加该功能：
