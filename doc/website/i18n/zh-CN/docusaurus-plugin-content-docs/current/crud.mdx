...
如[介绍章节](code-gen.md)所述，对schema运行`ent`将生成以下资源：
...
**SaveX**保存宠物；与**Save**不同，**SaveX**在出错时会触发panic
...
在某些项目中，"批量更新"操作被禁止并通过hooks阻断。但仍需根据特定条件按ID更新单条记录，此时可按如下方式使用`Where`选项：
...
:::info
由于upsert API通过`ON CONFLICT`子句实现（MySQL中使用`ON DUPLICATE KEY`），Ent仅向数据库执行单条语句，因此此类操作仅会触发创建[钩子](hooks.md)
:::
...
更高级的遍历方法可参阅[下一章节](traversals.md)
...
选择部分对象和部分关联关系。获取所有宠物及其主人，但仅选择并填充`ID`和`Name`字段。
...
每个生成的节点类型都有专属的变更类型。例如所有[`User`构建器](crud.mdx#create-an-entity)共享同一个生成的`UserMutation`对象，但所有构建器类型都实现了通用的<a target="_blank" href="https://pkg.go.dev/entgo.io/ent?tab=doc#Mutation">`ent.Mutation`</a>接口。

例如要为`ent.UserCreate`和`ent.UserUpdate`编写通用代码，可使用`UserMutation`对象：
...
若需对多种类型应用相同方法集，可使用通用`ent.Mutation`接口或自定义接口实现。
