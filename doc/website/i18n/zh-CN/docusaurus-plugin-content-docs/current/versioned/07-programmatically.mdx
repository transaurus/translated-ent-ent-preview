...
本节描述的变更可在配套代码库的PR [#2](https://github.com/rotemtam/ent-versioned-migrations-demo/pull/2/files)中找到。

:::

第一步是通过传入`sql/versioned-migration`功能标志来启用版本化迁移功能。根据执行Ent代码生成器的方式，您需要选择以下两种方式之一：
...
运行代码生成后，您会看到以下[新增方法](https://github.com/rotemtam/ent-versioned-migrations-demo/commit/e724fa32330d920fd405b9785fcfece2a46ea56c#diff-370235e5107bbdd35861063f3beff1507723ebdda6e29a39cdde1f1a944594d8)出现在`ent/migrate/migrate.go`文件中：
...
本节描述的变更可在配套代码库的PR [#4](https://github.com/rotemtam/ent-versioned-migrations-demo/pull/4/files)中找到。

:::

### 开发数据库

为了能规划出准确且一致的迁移文件，Atlas引入了[开发数据库](https://atlasgo.io/concepts/dev-database)的概念——这是一个临时数据库，用于模拟数据库在不同变更后的状态。因此，要使用Atlas自动规划迁移，我们需要在迁移规划脚本中提供该数据库的连接字符串。通常我们会使用本地Docker容器来启动这样的数据库。现在运行以下命令进行配置：

```shell
docker run --rm --name atlas-db-dev -d -p 3306:3306 -e MYSQL_DATABASE=dev -e MYSQL_ROOT_PASSWORD=pass mysql:8
```

使用刚配置好的开发数据库，我们可以编写一个利用Atlas规划迁移文件的脚本。在项目的`ent/migrate`目录下新建名为`main.go`的文件：
...
注意您需要在高亮行处对脚本进行修改：调整`migrate`包的导入路径以匹配您的项目，并提供开发数据库的连接字符串。

:::

要运行该脚本，首先在项目的`ent/migrate`目录下创建`migrations`文件夹：
...
运行脚本后，您会发现在`ent/migrate/migrations`目录下生成了两个新文件。第一个是名为`atlas.sum`的校验文件，Atlas用它来确保迁移历史的线性一致性：
