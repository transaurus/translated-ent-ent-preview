...
Ent 支持操作数据库视图。与通常由表支撑的常规 Ent 类型（模式）不同，视图作为"虚拟表"存在，其数据来源于查询结果。以下示例展示了如何在 Ent 中定义 `VIEW`。更多细节选项请参阅指南后续内容。
...
- 与 `ent.Schema` 不同，`ent.View` 默认不包含 `ID` 字段。如需在视图中包含 `id` 字段，需显式定义为字段。
- 钩子无法注册到视图，因其只读特性。
- Atlas 为 Ent 视图提供原生支持，包括版本化迁移和测试。但若未使用 Atlas，需手动管理视图迁移，因 Ent 不提供其模式迁移功能。
:::

## 简介

定义在 `ent/schema` 包中的视图需嵌入 `ent.View` 类型而非 `ent.Schema` 类型。除字段外，视图可包含边、拦截器和注解以实现额外集成。例如：
...
定义视图模式后，需向 Ent（及 Atlas）声明构成该视图的 SQL 查询。若未配置，执行如上述定义的 Ent 查询将因缺少 `clean_users` 表而失败。

:::note[Atlas 指南]
本文档后续内容假设您使用 Ent 配合 [Atlas Pro](https://atlasgo.io/features#pro-plan)，因 Ent 本身不支持视图或其他非表数据库对象的迁移。但使用 Atlas 或其专业订阅<u>并非强制</u>。只要视图存在于数据库中，Ent 客户端即可查询。
:::

配置视图定义 (`AS SELECT ...`) 有两种方案：

1. 在 Go 代码的 `ent/schema` 内定义
2. 保持 `ent/schema` 独立于视图定义，通过 Atlas 手动或自动创建外部定义

以下分别探讨这两种方案：

### Go 代码定义

本示例展示如何在 Ent 模式中定义包含 SQL 定义 (`AS ...`) 的 `ent.View`。

此方案主要优势在于：`CREATE VIEW` 的正确性会在迁移阶段而非查询阶段被验证。例如，若 `ent/schema` 中定义的 `ent.Field` 在 SQL 定义中不存在，PostgreSQL 将返回如下错误：
...
通过创建包含必要参数的 `atlas.hcl` 文件简化配置，该配置文件将在下文[使用](#usage)章节中使用：
...
完整示例参见 [Ent 代码库](https://github.com/ent/ent/tree/master/examples/viewschema)。
...
在Ent中定义视图模式后，需要单独配置（或创建）SQL的`CREATE VIEW`定义，以确保在Ent运行时查询时该视图存在于数据库中。
...
接下来，我们创建一个`atlas.hcl`配置文件，其中包含一个`composite_schema`，该模式同时包含我们的`ent/schema`和`schema.sql`文件：
