...
若要将`id`字段配置为在所有表中唯一，请在运行模式迁移时使用[WithGlobalUniqueID](migrate.md#universal-ids)选项。

如果需要为`id`字段进行不同的配置，或者需要在应用程序创建实体时提供`id`值（例如UUID），可以覆盖内置的`id`配置。例如：
...
自定义类型必须是可以转换为Go基本类型的类型，或者实现了[ValueScanner](https://pkg.go.dev/entgo.io/ent/schema/field?tab=doc#ValueScanner)接口的类型，或者具有[外部ValueScanner](#external-valuescanner)。此外，如果提供的类型实现了Validator接口且未设置验证器，则将使用类型验证器。
...
可通过[`entsql.Annotation`](https://pkg.go.dev/entgo.io/ent@master/dialect/entsql#Annotation)为默认值配置添加SQL专用字面量或表达式（如函数调用）：
...
若`DefaultFunc`返回错误，建议通过[schema-hooks](hooks.md#schema-hooks)妥善处理，详见[此FAQ](faq.md#how-to-use-a-custom-generator-of-ids)

## 字段验证器

字段验证器是形如`func(T) error`的函数，通过`Validate`方法在模式中定义，会在创建/更新实体前对字段值进行校验

支持的验证器类型包括`string`及所有数值类型
...
可选字段在实体创建时非必须，在数据库中会设置为可空字段。注意：**字段默认必填**，需显式调用`Optional`方法设为可选
...
当需要区分字段零值与`nil`时（如数据库列含`0`或`NULL`），应使用`Nillable`选项

对类型为`T`的`Optional`字段启用`Nillable`后，生成的结构体字段类型变为`*T`。数据库返回`NULL`时字段值为`nil`，否则为指向实际值的指针
...
`Nillable`字段还能避免未`Select`查询的字段在JSON序列化时出现零值，例如`time.Time`字段：
...
通过`Deprecated`方法可标记字段为弃用状态。弃用字段在查询时默认不会被选中，且生成代码中会为这些字段添加`Deprecated`注解。
...
索引可在多字段和某些边类型上定义，但需注意该特性目前仅支持SQL数据库。

详见[索引](schema-indexes.md)章节。

## 结构体标签

通过`StructTag`方法可为生成的实体添加自定义结构体标签。若未提供该选项，或提供的标签未包含`json`标签，系统会自动添加带字段名的默认`json`标签。
...
默认情况下，`ent`仅生成`schema.Fields`方法中配置的字段作为实体模型。例如给定如下模式配置：
...
若需添加不存储到数据库的附加字段，请使用[外部模板](code-gen.md/#external-templates)。例如：
...
通过`Sensitive`方法可将字符串字段标记为敏感字段。敏感字段不会被打印输出，且在编码时会被忽略。
...
默认情况下，Ent在**PostgreSQL和SQLite**中使用简单字符串类型表示枚举值。但在某些场景下，可能需要使用数据库提供的原生枚举类型。具体操作请参考[枚举迁移指南](/docs/migration/enum-types)。
:::

当使用自定义[`GoType`](#go-type)时，该类型必须可转换为基本`string`类型，或需实现[ValueScanner](https://pkg.go.dev/entgo.io/ent/schema/field#ValueScanner)接口。

自定义Go类型还需实现[EnumValues](https://pkg.go.dev/entgo.io/ent/schema/field#EnumValues)接口，用于告知Ent枚举的允许值范围。
...
`Annotations`用于在代码生成时为字段对象附加任意元数据。模板扩展可以获取这些元数据并在模板中使用。
