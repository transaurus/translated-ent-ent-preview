...
每个查询构建器都有一系列以 `With<E>(...func(<N>Query))` 形式命名的方法，对应其每条边。其中`<E>`代表边名称（如`WithGroups`），`<N>`代表边类型（如`GroupQuery`）。

请注意该功能仅支持SQL方言。

## 命名边查询

某些场景下需要以自定义名称预加载边。例如GraphQL查询中，两个别名引用同一条边但使用不同参数。为此Ent提供了`WithNamed<E>` API，需通过[`namedges`](features.md#named-edges)特性标志启用，并可无缝集成[EntGQL字段集合](tutorial-todo-gql-field-collection.md)功能。
