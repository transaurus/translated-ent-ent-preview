...
迁移通常用于更改数据库模式，但在某些情况下需要修改数据库中存储的数据。例如添加种子数据，或使用自定义默认值回填空列。

此类迁移称为数据迁移。本文将讨论如何利用Ent规划数据迁移，并将其集成到常规的模式迁移工作流中。
...
使用版本化迁移时，数据迁移应存储在相同的`migrations`目录中，并与常规迁移以相同方式执行。但建议将数据迁移与模式迁移分开存储，以便于测试。

此类迁移采用SQL格式，即使Ent模式发生修改且生成代码不再兼容数据迁移文件，仍可安全执行（且存储内容保持不变）。

创建数据迁移脚本有两种方式：手动编写和自动生成。手动编写时用户需撰写所有SQL语句以精确控制执行内容；也可使用Ent自动生成数据迁移。建议验证生成文件的正确性，某些情况下可能需手动修正。

### 手动创建

1\. 若未安装Atlas，请参阅其[入门指南](https://atlasgo.io/getting-started/#installation)
...
4\. 更新迁移目录[完整性文件](https://atlasgo.io/concepts/migration-directory-integrity)：
...
当前Ent提供数据迁移文件的初步生成支持。该功能可简化大多数情况下复杂SQL语句的手动编写过程。但仍建议验证生成文件的正确性，极端情况下可能需手动编辑。

1\. 若未配置，请先创建[版本化迁移设置](/docs/versioned/intro)
...
3\. 若生成文件被编辑，需通过以下命令更新迁移目录[完整性文件](https://atlasgo.io/concepts/migration-directory-integrity)：
...
添加迁移文件后，强烈建议在本地数据库应用以验证其有效性及预期结果。该过程可手动完成或通过程序自动化。
...
3\. 在本地数据库填充临时数据以模拟执行数据迁移前的生产环境状态。
