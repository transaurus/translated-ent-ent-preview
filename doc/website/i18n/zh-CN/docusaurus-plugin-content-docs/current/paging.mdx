...
`Order` 也可用于根据实体关联边的数量进行排序。例如，以下查询返回所有用户并按他们拥有的帖子数量排序：
...
实体也可根据边字段的值进行排序。例如，以下查询返回所有帖子并按作者姓名排序：
...
生成的边排序函数支持自定义条件。例如，以下查询返回所有用户并按他们帖子点赞数与浏览数之和排序：
...
像 `SUM()` 和 `COUNT()` 这样的聚合条件并未在模式中定义，因此不会出现在生成的实体上。但有时需要获取这些信息用于用户展示或实现基于游标的分页。每个实体上定义的 `Value` 方法允许在查询中选择排序值时获取该值：
